...
path_kup_hyperc3_code = "/users/jvargh7/projects/kiosk-user-patterns-hyperc3/code"
path_kup_hyperc3_folder = "/users/jvargh7/projects/kiosk-user-patterns-hyperc3"
path_brfss_parquet_hyperc3_folder = "/group/jvargh7-g00/data/brfss"
...
library(arrow)
brfss2022 = open_dataset(paste0(path_brfss_parquet_hyperc3_folder, "/year=2022/"), format = "parquet") %>%
...
stratified_estimates = future_map_dfr(c("age_group_pursuant","sex","raceeth","urban","state_names"),
                                      
                                      function(v){
                                        library(arrow) # Has to be loaded inside to prevent some nng error - ChatGPT suggested fix
                                        library(srvyr)	
                                        ...
                                        brfss2022_svy <- brfss2022 %>% 
                                          ...
                                          srvyr::filter(sex == "Male" |(sex == "Female" & PREGNANT %in% c(2,7,9)) | (sex == "Female" & is.na(PREGNANT))) %>%
 ...
 write_csv(stratified_estimates, file = paste0(path_kup_hyperc3_folder,"/working/kuphyc302_brfss stratified estimates of overweight and obesity in parallel.csv"))
