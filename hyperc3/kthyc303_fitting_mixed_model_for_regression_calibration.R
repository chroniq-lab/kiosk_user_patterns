...
TYPE = c("obesity_corrected","overweight_corrected")

for (y_r in YEAR_RANGE){
  
  dt = readRDS(paste0(path_kinetic_t2d_hyperc3_folder,"/working/input/ktsens02 regression calibration dataset.RDS")) %>%
  ...
  for(outcome_var in TYPE){
    ...
    path_results = paste0(path_kinetic_t2d_hyperc3_folder,"/working/output/kthyc303")
