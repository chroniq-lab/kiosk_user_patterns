...
for (y_r in YEAR_RANGE){
  ...
  dt = readRDS(paste0(path_kinetic_t2d_hyperc3_folder,"/working/input/ktana08 gbd comparison dataset.RDS")) %>%
  ...
  for(outcome_var in TYPE){
    ...
    formula = as.formula(paste0(outcome_var," ~ (1 |state/FIPS) + age_group_gbd + ethnicity_updated*gender + urban",spatial_covariates))
    ...
    path_results = paste0(path_kinetic_t2d_hyperc3_folder,"/working/output/kthyc302")
    if(!dir.exists(path_results)){
      dir.create(path_results)
    }
    
    filename <- paste0("glmer_gbdcompare_",paste0(y_r,collapse=""), "_", outcome_var, ".RDS")
