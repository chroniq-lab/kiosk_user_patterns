...
brfss_clean <- brfss2022 %>%
  mutate(
    ...
    rural = as.numeric(`_METSTAT` == 2),  # 1 = rural, 0 = not rural
    pregnant_num = as.numeric(PREGNANT)   # 1 = pregnant, 2 = uncertain, 0 = not pregnant
  ) %>%
  filter(
    !is.na(female), !is.na(age_group), !is.na(race),
    !is.na(height), !is.na(weight), !is.na(bmi), !is.na(diabetes), !is.na(rural),
    is.na(pregnant_num) | pregnant_num == 0   # Exclude if currently pregnant (pregnant_num == 1)
  )
