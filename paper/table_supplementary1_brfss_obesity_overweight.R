...
brfss_clean <- brfss2022 %>%
  mutate(
    ...
    race_cat = case_when(
      ...
      `_RACEPR1` == 7 ~ "Hispanic",
      `_RACEPR1` %in% c(3, 4, 5, 6, 8, 9) ~ "Non-Hispanic Other"
    ),
    age_cat = case_when(
    ...
    bmi = `_BMI5` / 100,
    height = HTM4,             # <-- add these two lines!
    weight = WTKG3 / 100,
  ...
  filter(
    !is.na(female), !is.na(age_cat), !is.na(race_cat),
...
# ---------- BY RACE/ETHNICITY ----------
race_order <- c("Hispanic", "Non-Hispanic White", "Non-Hispanic Black", "Non-Hispanic Other")
